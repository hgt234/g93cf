/*
 * Flow Designer custom Action for the "Hybrid GPU Desktop" catalog item.
 * Inputs: ritm_number, ritm_sys_id, requested_for_upn
 * Outputs: hostname, platform, ado_run_id, ado_run_url
 *
 * Configure a separate REST Message named "Azure DevOps AVD Hybrid" with a
 * POST method named "Queue pipeline" and credentials in a connection alias.
 */
(function execute(inputs, outputs) {
    ...
    var request = new sn_ws.RESTMessageV2('Azure DevOps AVD Hybrid', 'Queue pipeline');
    ...
    if (status !== 200 && status !== 201) {
        throw new Error('Azure DevOps hybrid queue request failed with HTTP ' + status + ': ' + response.getErrorMessage());
    }

    var body = JSON.parse(response.getBody());
    outputs.hostname = hostname;
    outputs.platform = 'Hybrid GPU';
